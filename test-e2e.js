...
(async () => {
  ...
  // 6c. Ajout / suppression de questions
  ...
  admin.emit('admin:deleteQuestion', nq - 1);
  await wait(150);
  assert(adminState.questions.length === nq - 1, 'question supprimée');
  admin.emit('admin:deleteQuestion', 0); // déjà jouée → doit être refusée
  await wait(150);
